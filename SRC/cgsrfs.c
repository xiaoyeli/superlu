	...
	*(unsigned char *)transc = 'C';
	...
	while (1) { /* Loop until stopping criterion is satisfied. */
...
#else
	    ccopy_((int*)&A->nrow, Bptr, (int*)&ione, work, (int*)&ione);
#endif
		...
		CAXPY(&A->nrow, &done, work, &ione,
		       &Xmat[j*ldx], &ione);
#else
		caxpy_((int*)&A->nrow, &done, work, (int*)&ione,
		       &Xmat[j*ldx], (int*)&ione);
#endif
		lstres = berr[j];
		++count;
	...
	do {
	    clacon2_((int*)&A->nrow, &work[A->nrow], work, &ferr[j], (int*)&kase, (int*)isave);
	    if (kase == 0) break;
