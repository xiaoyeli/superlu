...
/* Internal prototypes */
void  *cexpand (int_t *, MemType,int_t, int_t, GlobalLU_t *);
int_t   cLUWorkInit (int_t, int_t, int_t, int_t **, complex **, GlobalLU_t *);
void  copy_mem_complex (int_t, void *, void *);
void  cStackCompress (GlobalLU_t *);
void  cSetupSpace (void *, int_t, GlobalLU_t *);
void  *cuser_malloc (int_t, int_t, GlobalLU_t *);
void  cuser_free (int_t, int_t, GlobalLU_t *);
...
void *cuser_malloc(int_t bytes, int_t which_end, GlobalLU_t *Glu)
{
    void *buf;
...
void cuser_free(int_t bytes, int_t which_end, GlobalLU_t *Glu)
{
    if ( which_end == HEAD ) {
	Glu->stack.top1 -= bytes;
    } else {
 ...
 * </pre>
 */
int_t cQuerySpace(SuperMatrix *L, SuperMatrix *U, mem_usage_t *mem_usage)
{
    SCformat *Lstore;
    NCformat *Ustore;
 ...
 * </pre>
 */
int_t ilu_cQuerySpace(SuperMatrix *L, SuperMatrix *U, mem_usage_t *mem_usage)
{
    SCformat *Lstore;
    NCformat *Ustore;
 ...
 */
int_t
cLUMemInit(fact_t fact, void *work, int_t lwork, int_t m, int_t n, int_t annz,
	  int_t panel_size, float fill_ratio, SuperMatrix *L, SuperMatrix *U,
          GlobalLU_t *Glu, int_t **iwork, complex **dwork)
{
	...
	if ( Glu->MemModel == SYSTEM ) {
	    ...
	    xsup   = (int_t *)cuser_malloc((n+1) * iword, HEAD, Glu);
	    supno  = (int_t *)cuser_malloc((n+1) * iword, HEAD, Glu);
	    xlsub  = (int_t *)cuser_malloc((n+1) * iword, HEAD, Glu);
	    xlusup = (int_t *)cuser_malloc((n+1) * iword, HEAD, Glu);
	    xusub  = (int_t *)cuser_malloc((n+1) * iword, HEAD, Glu);
	}
 ...
 */
void
cSetRWork(int_t m, int_t panel_size, complex *dworkptr,
 ...
 */
void cLUWorkFree(int_t *iwork, complex *dwork, GlobalLU_t *Glu)
{
    if ( Glu->MemModel == SYSTEM ) {
	SUPERLU_FREE (iwork);
	SUPERLU_FREE (dwork);
