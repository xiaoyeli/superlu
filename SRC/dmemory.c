...
/* Internal prototypes */
void  *dexpand (int_t *, MemType,int_t, int_t, GlobalLU_t *);
int_t   dLUWorkInit (int_t, int_t, int_t, int_t **, double **, GlobalLU_t *);
void  copy_mem_double (int_t, void *, void *);
void  dStackCompress (GlobalLU_t *);
void  dSetupSpace (void *, int_t, GlobalLU_t *);
void  *duser_malloc (int_t, int_t, GlobalLU_t *);
void  duser_free (int_t, int_t, GlobalLU_t *);
...
void *duser_malloc(int_t bytes, int_t which_end, GlobalLU_t *Glu)
{
    void *buf;
...
void duser_free(int_t bytes, int_t which_end, GlobalLU_t *Glu)
{
    if ( which_end == HEAD ) {
	Glu->stack.top1 -= bytes;
    } else {
 ...
 * </pre>
 */
int_t dQuerySpace(SuperMatrix *L, SuperMatrix *U, mem_usage_t *mem_usage)
{
    SCformat *Lstore;
    NCformat *Ustore;
 ...
 * </pre>
 */
int_t ilu_dQuerySpace(SuperMatrix *L, SuperMatrix *U, mem_usage_t *mem_usage)
{
    SCformat *Lstore;
    NCformat *Ustore;
 ...
 */
int_t
dLUMemInit(fact_t fact, void *work, int_t lwork, int_t m, int_t n, int_t annz,
	  int_t panel_size, double fill_ratio, SuperMatrix *L, SuperMatrix *U,
          GlobalLU_t *Glu, int_t **iwork, double **dwork)
{
	...
	if ( Glu->MemModel == SYSTEM ) {
	    ...
	    xsup   = (int_t *)duser_malloc((n+1) * iword, HEAD, Glu);
	    supno  = (int_t *)duser_malloc((n+1) * iword, HEAD, Glu);
	    xlsub  = (int_t *)duser_malloc((n+1) * iword, HEAD, Glu);
	    xlusup = (int_t *)duser_malloc((n+1) * iword, HEAD, Glu);
	    xusub  = (int_t *)duser_malloc((n+1) * iword, HEAD, Glu);
	}
...
int_t
dLUWorkInit(int_t m, int_t n, int_t panel_size, int_t **iworkptr, 
 ...
 */
void
dSetRWork(int_t m, int_t panel_size, double *dworkptr,
 ...
 */
void dLUWorkFree(int_t *iwork, double *dwork, GlobalLU_t *Glu)
{
    if ( Glu->MemModel == SYSTEM ) {
	SUPERLU_FREE (iwork);
	SUPERLU_FREE (dwork);
